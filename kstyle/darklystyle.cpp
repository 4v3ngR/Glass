...
#include "darklystyle.h"

#include "darkly.h"
#include "darklyanimations.h"
#include "darklyblurhelper.h"
#include "darklyframeshadow.h"
#include "darklymdiwindowshadow.h"
#include "darklymnemonics.h"
#include "darklypropertynames.h"
#include "darklyshadowhelper.h"
#include "darklysplitterproxy.h"
#include "darklystyleconfigdata.h"
#include "darklytoolsareamanager.h"
#include "darklywidgetexplorer.h"
#include "darklywindowmanager.h"
...
private:
    ...
    Darkly::WeakPointer<const QWidget> _tabBar;
};
...
public:
    ...
    explicit ComboBoxItemDelegate(QAbstractItemView *parent)
        ...
        , _itemMargin(Darkly::Metrics::ItemView_ItemMarginWidth)
    {
    }
...
private:
    //* proxy
    Darkly::WeakPointer<QAbstractItemDelegate> _proxy;
...
Style::Style()
    ...
    , _tabBarData(new DarklyPrivate::TabBarData(this))
#if DARKLY_HAVE_KSTYLE
...
#endif
{
    ...
    dbus.connect(QString(),
                 QStringLiteral("/DarklyStyle"),
                 QStringLiteral("org.kde.Darkly.Style"),
                 QStringLiteral("reparseConfiguration"),
                 this,
                 SLOT(configurationChanged()));

    dbus.connect(QString(),
                 QStringLiteral("/DarklyDecoration"),
                 QStringLiteral("org.kde.Darkly.Style"),
                 QStringLiteral("reparseConfiguration"),
                 this,
                 SLOT(configurationChanged()));
...
void Style::unpolish(QWidget *widget)
{
    ...
    if (DarklyPrivate::possibleTranslucentToolBars.contains(widget))
        DarklyPrivate::possibleTranslucentToolBars.remove(widget);
...
QRect Style::progressBarGrooveRect(const QStyleOption *option, const QWidget *widget) const
{
    ...
    const bool horizontal(DarklyPrivate::isProgressBarHorizontal(progressBarOption));
...
bool Style::isStylableToolbar(const QWidget *w, bool allowInvisible) const // should be in helper
{
    ...
    if (QMainWindow *mw = qobject_cast<QMainWindow *>(p)) {
        ...
        if (tb->orientation() == Qt::Vertical) {
            if (tb->y() == 0) {
                if (DarklyPrivate::possibleTranslucentToolBars.size() == 0) {
                    DarklyPrivate::possibleTranslucentToolBars.insert(w);
                    return true;
                } else if (DarklyPrivate::possibleTranslucentToolBars.contains(w) && DarklyPrivate::possibleTranslucentToolBars.size() == 1)
                    return true;
