...
#include "glass.h"
#include "glassanimations.h"
#include "glassblurhelper.h"
#include "glassframeshadow.h"
#include "glassmdiwindowshadow.h"
#include "glassmnemonics.h"
#include "glasspropertynames.h"
#include "glassshadowhelper.h"
#include "glasssplitterproxy.h"
#include "glassstyleconfigdata.h"
#include "glasstoolsareamanager.h"
#include "glasswidgetexplorer.h"
#include "glasswindowmanager.h"
...
private:
    ...
    Glass::WeakPointer<const QWidget> _tabBar;
};
...
public:
    ...
    explicit ComboBoxItemDelegate(QAbstractItemView *parent)
        ...
        , _itemMargin(Glass::Metrics::ItemView_ItemMarginWidth)
    {
    }
...
private:
    //* proxy
    Glass::WeakPointer<QAbstractItemDelegate> _proxy;
...
Style::Style()
    ...
    , _tabBarData(new GlassPrivate::TabBarData(this))
#if GLASS_HAVE_KSTYLE
    , SH_ArgbDndWindow(newStyleHint(QStringLiteral("SH_ArgbDndWindow")))
    , CE_CapacityBar(newControlElement(QStringLiteral("CE_CapacityBar")))
#endif
{
    // use DBus connection to update on glass configuration change
    auto dbus = QDBusConnection::sessionBus();
    dbus.connect(QString(),
                 QStringLiteral("/GlassStyle"),
                 QStringLiteral("org.kde.Glass.Style"),
                 QStringLiteral("reparseConfiguration"),
                 this,
                 SLOT(configurationChanged()));

    dbus.connect(QString(),
                 QStringLiteral("/GlassDecoration"),
                 QStringLiteral("org.kde.Glass.Style"),
                 QStringLiteral("reparseConfiguration"),
                 this,
                 SLOT(configurationChanged()));
...
void Style::polish(QApplication *app)
{
    ...
    if (appName == "Qt-subapplication")
        ...
        _isKdevelop = true;
    else if (appName == "plasma" || appName.startsWith("plasma-") || appName == "plasmashell" // Plasma5
...
void Style::unpolish(QWidget *widget)
{
    ...
    if (GlassPrivate::possibleTranslucentToolBars.contains(widget))
        GlassPrivate::possibleTranslucentToolBars.remove(widget);
...
int Style::pixelMetric(PixelMetric metric, const QStyleOption *option, const QWidget *widget) const
{
    ...
    case PM_DefaultFrameWidth: {
        ...
        if (widget && widget->inherits("KTextEditor::View") && !StyleConfigData::kTextEditDrawFrame() && !_isKdevelop)
            return 0;
                ...
                if (StyleConfigData::dolphinSidebarOpacity() < (palette.color(QPalette::Window).alpha() / 255) * 100) {
                    ...
                    if (tabWidget) {
                        if (tabWidget->x() < dockWidget->x())
                            shadowRect = QRect(rect.topLeft() - QPoint(29, 0), QSize(30, rect.height()));
                    }
...
QRect Style::progressBarGrooveRect(const QStyleOption *option, const QWidget *widget) const
{
    ...
    const bool horizontal(GlassPrivate::isProgressBarHorizontal(progressBarOption));
...
QSize Style::tabBarTabSizeFromContents(const QStyleOption *option, const QSize &contentsSize, const QWidget *) const
{
    ...
    if (verticalTabs) {
        ...
        if (hasIcon && !hasText)
            ...
            size =
                size.expandedTo(QSize(Metrics::TabBar_TabMinWidth, Metrics::TabBar_TabMinHeight + StyleConfigData::tabsHeight() + (documentMode ? 0 : 2 * 4)));
    }
...
bool Style::drawFramePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const bool isTitleWidget(StyleConfigData::titleWidgetDrawFrame() && widget && widget->parent() && widget->parent()->inherits("KTitleWidget"));
...
bool Style::drawPanelItemViewItemPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (widget) {
        if (!widget->property(PropertyNames::sidePanelView).toBool()) {
            if (!(qobject_cast<const QTableView *>(widget) || qobject_cast<const QListWidget *>(widget) || qobject_cast<const QListView *>(widget))) {
                Corners corners;
                if (!viewItemOption->rect.isNull()) {
                    if (viewItemOption->viewItemPosition == QStyleOptionViewItem::Beginning
                        ...
                        corners |= CornersLeft;
                    if (viewItemOption->viewItemPosition == QStyleOptionViewItem::End || viewItemOption->viewItemPosition == QStyleOptionViewItem::OnlyOne)
...
bool Style::drawToolButtonLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (hasText && (!(hasArrow || hasIcon) || toolButtonOption->toolButtonStyle == Qt::ToolButtonTextOnly)) {
        ...
        iconRect = QRect(
            ...
            iconSize);
        textRect = QRect(QPoint(contentsRect.left() + (contentsRect.width() - textSize.width()) / 2, iconRect.bottom() + Metrics::ToolButton_ItemSpacing + 1),
                         textSize);
        textFlags |= Qt::AlignCenter;
    ...
    if (hasText && textRect.isValid()) {
        ...
        if (flat)
            textRole = (((hasFocus && sunken) || (state & State_Sunken)) && !mouseOver) ? QPalette::HighlightedText : QPalette::WindowText;
...
bool Style::drawMenuBarEmptyAreaControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (widget && _helper->titleBarColor(windowActive).alphaF() * 100.0 < 100 && _translucentWidgets.contains(widget->window())) {
        ...
        if (GlassPrivate::possibleTranslucentToolBars.isEmpty())
    ...
    if (option->state & State_Horizontal) {
        ...
        if (bellowMenuBar || widget->y() == 0) {
            ...
            if (_isDolphin && StyleConfigData::dolphinSidebarOpacity() < 100) {
                QList<QWidget *> sidebars =
                    widget->window()->findChildren<QWidget *>(QRegularExpression("^(places|terminal|info|folders)Dock$"), Qt::FindDirectChildrenOnly);
        ...
        // overwrite parent style behaviour and add horizontal padding
            ...
            if (!tabOption->icon.isNull()) {
                QPixmap tabIcon = tabOption->icon.pixmap(tabOption->iconSize,
                                                         (tabOption->state & State_Enabled) ? QIcon::Normal : QIcon::Disabled,
...
bool Style::drawTabBarTabShapeControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (selected)
        if (StyleConfigData::tabUseHighlightColor()) {
            ...
            color =
                (documentMode && !isQtQuickControl && !hasAlteredBackground(widget)) ? palette.color(QPalette::Window) : _helper->frameBackgroundColor(palette);
        }
    else {
    ...
    if (StyleConfigData::oldTabbar()) {
        ...
        case QTabBar::TriangularEast:
            ...
            if (selected) {
                ...
                if (isRightOfSelected)
                    rect.adjust(0, -StyleConfigData::cornerRadius(), -StyleConfigData::cornerRadius(), 0);
                if (isLeftOfSelected)
                    rect.adjust(0, -StyleConfigData::cornerRadius(), 0, StyleConfigData::cornerRadius());
                else if (!isLast)
                    rect.adjust(0, 0, 0, overlap);
            }
            break;
    ...
    if (StyleConfigData::tabBarAltStyle()) {
        ...
        painter->setRenderHint(QPainter::Antialiasing, true);
        painter->drawRoundedRect(backgroundRect.adjusted(side == SideLeft                       ? backgroundRect.width() - (hightlightHeight + offset)
                                                             : (side == SideRight && !selected) ? 1
                                                                                                : 0,
                                                         side == SideTop                         ? backgroundRect.height() - (hightlightHeight + offset)
                                                             : (side == SideBottom && !selected) ? 1
                                                                                                 : 0,
                                                         side == SideRight                     ? -backgroundRect.width() + (hightlightHeight + offset)
                                                             : (side == SideLeft && !selected) ? -1
                                                                                               : 0,
                                                         side == SideBottom                   ? -backgroundRect.height() + (hightlightHeight + offset)
        ...
        if (documentMode) {
            ...
            if (unifiedTabAndHeader) {
                ...
                if (isFirst)
                    ...
                    rect.adjust(0, 0, 3, 0);
                if (color.alphaF() < 1)
                    rect.adjust(0, side == SideTop ? -3 : 0, 0, side == SideBottom ? 3 : 0);
...
bool Style::drawComboBoxComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (option->subControls & SC_ComboBoxArrow) {
        ...
        if (editable) {
            ...
            if (empty || !enabled)
                arrowColor = _helper->arrowColor(palette, QPalette::Disabled, QPalette::WindowText);
            ...
            arrowColor = _helper->arrowColor(palette, QPalette::Disabled, QPalette::ButtonText);
...
void Style::renderMenuTitle(const QStyleOptionToolButton *option, QPainter *painter, const QWidget *) const
{
    ...
    _helper->renderSeparator(
        painter,
        QRect(option->rect.bottomLeft() - QPoint(0, (Metrics::MenuItem_MarginHeight + StyleConfigData::menuItemHeight())), QSize(option->rect.width(), 1)),
        color);
...
bool Style::isStylableToolbar(const QWidget *w, bool allowInvisible) const // should be in helper
{
    ...
    if (!tb || w->autoFillBackground() || w->testAttribute(Qt::WA_StyleSheetTarget) // not drawn by Kvantum (CE_ToolBar may not be called)
        || _isPlasma) {
        return false;
    }
    ...
    if (QMainWindow *mw = qobject_cast<QMainWindow *>(p)) {
        ...
        if (tb->orientation() == Qt::Vertical) {
            if (tb->y() == 0) {
                if (GlassPrivate::possibleTranslucentToolBars.size() == 0) {
                    GlassPrivate::possibleTranslucentToolBars.insert(w);
                    return true;
                } else if (GlassPrivate::possibleTranslucentToolBars.contains(w) && GlassPrivate::possibleTranslucentToolBars.size() == 1)
                    return true;
        ...
        if (QWidget *mb = mw->menuWidget()) // WARNING: an empty menubar may be created by menuBar()
        {
            ...
            } else if (tb->y() == 0 && (allowInvisible || tb->isVisible())) // FIXME: Why can KtoolBar be invisible here?
            {
                GlassPrivate::possibleTranslucentToolBars.insert(w);
                return true;
            } else
